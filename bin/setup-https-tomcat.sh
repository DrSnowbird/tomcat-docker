...
if [ "${KEYSTORE_PASSWORD}" == "" ]; then
    ...
    echo "========================================================================"
    echo "You can now access to this KeyStore Tomcat server using:"
    echo " "
...
if [ ! -d ${TOMCAT_HOME}/conf ]; then
    echo "***************************************************************"
    echo "******* ERROR: Missing ${TOMCAT_HOME}/conf directory *********"
    echo "***************************************************************"
    exit 1
fi
...
#######################################################################################################
#### ---- Setup HTTPS Port in ${CATALINA_WEBAPPS}/ROOT/WEB-INF/classes/webprotege.properties" ---- ####
#######################################################################################################
#### ---------------------------------------------
...
#### ---- webprotege.properties with TOMCAT_PORT_HTTPS=8443 ----
replaceKeyValue ${CATALINA_WEBAPPS}/ROOT/WEB-INF/classes/webprotege.properties "https.port" "${TOMCAT_PORT_HTTPS}"

#################################################################################
#### ---- Insert HTTPS Connector into $TOMCAT_HOME/conf/server.xml file ---- ####
#################################################################################
echo "***************************************************************"
echo "******* Config Tomcat: ${TOMCAT_HOME}/conf/server.xml *********"
echo "***************************************************************"
...
if [ -s "${TOMCAT_HOME}/conf/server.xml" ]; then
    sed -i '/<Connector port=\"8080\" protocol/i\ '"\n${HTTPS_CONNECTOR}"'\n' ${TOMCAT_HOME}/conf/server.xml 
    echo "Done:\n ${HTTPS_CONNECTOR}" > ${TOMCAT_HOME}/conf/.enabled-https-setup.txt
fi
