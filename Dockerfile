...
ENV TOMCAT_MINOR_VERSION=${TOMCAT_MINOR_VERSION:-9.0.58}
...
RUN wget -q --no-check-certificate $(curl -k -s https://tomcat.apache.org/download-90.cgi | grep '.tar.gz' | head -1 | cut -d'"' -f2) && \
